...
const MotionTitle = (props) => {
    ...
    useEffect(() => {
        if (hover) {
            titleRef.current.style.transform = `perspective(300px) scale(1.4) rotateY(${mouse.x / 5 + 'deg'}) rotateX(${-mouse.y / 2 + 'deg'})`
    ...
    const handleMouse = (e) => {
        setMouse({ x: e.nativeEvent.offsetX - e.target.clientWidth / 2, y: e.nativeEvent.offsetY - e.target.clientHeight / 2 })
    }

    return (
        <Box p={pad} cursor={'move'}  onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onMouseMove={handleMouse}>
            ...
            <Heading transition="all 0.2s" ref={titleRef} as="h1" size="md" letterSpacing={'tighter'}   >
                {text}
            </Heading>
            <Text ref={textRef} transition="all 0.2s" py={[3, 0]}>Full-stack Interactive Developer</Text>
        </Box>
    )
}
